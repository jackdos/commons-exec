...
public class LogOutputStreamTest {
    ...
    @Test
    public void testWhereStdoutContainsUTF8Characters() throws Exception {
        this.systemOut = new CharsetAwareLogOutputStream(1, Charset.forName("UTF-8"));
        this.exec.setStreamHandler(new PumpStreamHandler(systemOut, systemOut));

        final CommandLine cl = new CommandLine(utf8CharacterScript);
        final int exitValue = exec.execute(cl);
        assertFalse(exec.isFailure(exitValue));
        assertEquals("This string contains UTF-8 characters like the see no evil monkey \uD83D\uDE48 and the right single quotation mark ’", ((CharsetAwareLogOutputStream) systemOut).getOutput());
    }

    @Test
    public void testSystemStdoutContainsUTF8Characters() throws Exception {
        this.systemOut = new CharsetAwareLogOutputStream(1);
        this.exec.setStreamHandler(new PumpStreamHandler(systemOut, systemOut));
    ...
    private class CharsetAwareLogOutputStream extends LogOutputStream {

        StringBuffer output = new StringBuffer();

        private CharsetAwareLogOutputStream(final int level) {
            super(level);
        }

        private CharsetAwareLogOutputStream(final int level, final Charset charset) {
            super(level, charset);
        }
